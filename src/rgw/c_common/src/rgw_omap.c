...
int rgw_omap_get(rados_ioctx_t ioctx,
    ...
    ret = rados_omap_get_next(iter, &got_key, &got_val, &got_val_len);
...
int rgw_omap_set(rados_ioctx_t ioctx,
    ...
    /* 设置值 - librados 12.2.7 使用 rados_write_op_omap_set */
    const char* const_keys[1] = { key };
    const char* const_vals[1] = { (const char*)val };
    const size_t lens[1] = { val_len };
    rados_write_op_omap_set(op, const_keys, const_vals, lens, 1);
...
int rgw_omap_del(rados_ioctx_t ioctx,
    ...
    /* 删除键 - 使用 librados 12.2.7 兼容 API */
    rados_write_op_omap_rm_keys(op, &key, 1);
...
int rgw_omap_set_multi(rados_ioctx_t ioctx,
    ...
    const char** vals = (const char**)malloc(sizeof(char*) * num_kvs);
    size_t* val_lens = (size_t*)malloc(sizeof(size_t) * num_kvs);

    if (!keys || !vals || !val_lens) {
        free(keys);
        free(vals);
        free(val_lens);
        rados_release_write_op(op);
    ...
    for (size_t i = 0; i < num_kvs; i++) {
        keys[i] = kvs[i].key;
        vals[i] = (const char*)kvs[i].val;
        val_lens[i] = kvs[i].val_len;
    }

    /* 设置值 - librados 12.2.7 使用 rados_write_op_omap_set */
    rados_write_op_omap_set(op, keys, vals, val_lens, num_kvs);
...
int rgw_omap_del_multi(rados_ioctx_t ioctx,
    ...
    /* 删除键 - librados 12.2.7 使用 rados_write_op_omap_rm_keys */
    rados_write_op_omap_rm_keys(op, keys, num_keys);
...
int rgw_omap_get_multi(rados_ioctx_t ioctx,
    ...
    /* librados 12.2.7 使用 rados_read_op_omap_get_vals_by_keys */
    rados_read_op_omap_get_vals_by_keys(op, keys, num_keys, &iter, &prval);
    ...
    while (true) {
        ...
        /* librados 12.2.7 兼容: rados_omap_get_next(iter, &key, &val, &val_len) */
        ret = rados_omap_get_next(iter, &got_key, &got_val, &got_val_len);
        if (ret < 0 || !got_key) {
            break;
        }
        ...
        /* 复制键值对 - key_len 通过 strlen 获取 */
        size_t got_key_len = strlen(got_key);
...
int rgw_omap_get_all(rados_ioctx_t ioctx,
    ...
    /* 获取迭代器 - librados 12.2.7 使用 rados_read_op_omap_get_vals */
    rados_omap_iter_t iter;
    int prval = 0;

    /* librados 12.2.7 使用 rados_read_op_omap_get_vals */
    rados_read_op_omap_get_vals(op, start_after, NULL,
                                 max_return > 0 ? max_return : RGW_OMAP_DEFAULT_PAGE_SIZE,
                                 &iter, &prval);
...
int rgw_omap_get_keys(rados_ioctx_t ioctx,
    ...
    *keys_count = 0;
    ...
    /* 获取迭代器 - librados 12.2.7 使用 rados_read_op_omap_get_keys */
    rados_omap_iter_t iter;
    int prval = 0;

    /* librados 12.2.7 使用 rados_read_op_omap_get_keys (deprecated but available) */
    rados_read_op_omap_get_keys(op, start_after, max_return,
                                 &iter, &prval);
...
int rgw_omap_cmp_and_set(rados_ioctx_t ioctx,
    ...
    /* 添加比较操作 - librados 12.2.7 使用 rados_write_op_omap_cmp */
    int prval = 0;
    rados_write_op_omap_cmp(op, key, LIBRADOS_CMPXATTR_OP_EQ,
                             (const char*)expected_val, expected_len, &prval);

    /* 如果比较成功则设置新值 - librados 12.2.7 使用 rados_write_op_omap_set */
    rados_write_op_omap_set(op, &key, (const char* const*)&new_val, &new_len, 1);
...
int rgw_omap_write_ctx_add(rgw_omap_write_ctx_t* ctx,
    ...
    /* librados 12.2.7 使用 rados_write_op_omap_set */
    const char* const_keys[1] = { key };
    const char* const_vals[1] = { (const char*)val };
    const size_t lens[1] = { val_len };
    rados_write_op_omap_set(ctx->op, const_keys, const_vals, lens, 1);
...
int rgw_omap_write_ctx_del(rgw_omap_write_ctx_t* ctx,
    ...
    /* librados 12.2.7 使用 rados_write_op_omap_rm_keys */
...
int rgw_omap_read_ctx_get_vals(rgw_omap_read_ctx_t* ctx,
    ...
    if (keys && num_keys > 0) {
        /* librados 12.2.7 使用 rados_read_op_omap_get_vals_by_keys */
        rados_read_op_omap_get_vals_by_keys(ctx->op, keys, num_keys, &iter, &prval);
    } else {
        /* librados 12.2.7 使用 rados_read_op_omap_get_vals */
        rados_read_op_omap_get_vals(ctx->op, NULL, NULL, RGW_OMAP_DEFAULT_PAGE_SIZE,
                                     &iter, &prval);
    }
    ...
    while (true) {
        ...
        /* librados 12.2.7 兼容: rados_omap_get_next(iter, &key, &val, &val_len) */
        int ret = rados_omap_get_next(iter, &got_key, &got_val, &got_val_len);
        if (ret < 0 || !got_key) {
            break;
        }
...
rgw_omap_iter_t* rgw_omap_iter_create(rados_ioctx_t ioctx,
    ...
    /* librados 12.2.7 使用 rados_read_op_omap_get_vals (忽略 filter_prefix) */
    (void)filter_prefix;  /* librados 12.2.7 不支持 filter_prefix */
    rados_read_op_omap_get_vals(op, start_after, NULL,
                                 max_return > 0 ? max_return : RGW_OMAP_DEFAULT_PAGE_SIZE,
                                 &iter, &prval);
...
int rgw_omap_iter_next(rgw_omap_iter_t* iter,
    ...
    /* librados 12.2.7 兼容: rados_omap_get_next(iter, &key, &val, &val_len) */
    int ret = rados_omap_get_next(iter->iter, &got_key, &got_val, &got_val_len);
    ...
    if (iter->cur_val) {
        memcpy(iter->cur_val, (uint8_t*)got_val, got_val_len);
...
int64_t rgw_omap_count(rados_ioctx_t ioctx, const char* oid) {
    ...
    /* 获取键迭代器 - librados 12.2.7 使用 rados_read_op_omap_get_keys */
    rados_omap_iter_t iter;
    int prval = 0;

    /* librados 12.2.7 使用 rados_read_op_omap_get_keys (deprecated but available) */
    rados_read_op_omap_get_keys(op, NULL, UINT64_MAX, &iter, &prval);
