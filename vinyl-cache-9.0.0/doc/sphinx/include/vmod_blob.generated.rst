...
call, allocating space for the string in Vinyl Cache workspaces.
...
`blob.transcode()`_ functions allocate Vinyl Cache workspace, as does
`blob.sub()`_ for the newly created BLOB.  If these functions are
failing, as indicated by "out of space" messages in the Vinyl Cache log
(with the ``VCL_Error`` tag), then you will need to increase the
`vinyld` parameters ``workspace_client`` and/or ``workspace_backend``.
...
need to increase the `vinyld` parameter ``thread_pool_stack``.
