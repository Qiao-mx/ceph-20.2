...
``sess_fail_other`` – `counter` - info
	...
	SessError log (vinyllog -g raw -i SessError).
...
``cache_hitpass`` – `counter` - info
	...
	Count of hits for pass. A cache hit for pass indicates that
	Vinyl Cache is going to pass the request to the backend and
	this decision has been cached in it self. This counts how
	many times the cached decision is being used.

``cache_hitmiss`` – `counter` - info

	Cache hits for miss.

	Count of hits for miss. A cache hit for miss indicates that
	Vinyl Cache is going to proceed as for a cache miss without
	request coalescing, and this decision has been cached. This
...
``backend_wait_fail`` – `counter` - info
...
``transit_stored`` – `counter` - info

	Stored uncacheable body bytes
...
``transit_buffered`` – `counter` - info

	Buffered uncacheable response body bytes
...
``n_test_gunzip`` – `counter` - info
	...
	Those operations occur when Vinyl Cache receives a compressed
...
``http1_absolute_form`` – `counter` - info

	Requests with absolute form request target

	Number of requests for which an absolute form request target (e.g.
	http://example.com/ or https://example.com/) was handled. The https://
	scheme is handled unless the feature ``https_scheme`` is disabled (see
	RUN TIME PARAMETERS in `vinyld(1)`).

	See also ReqTarget in `vsl(7)`. This counter is approximately correct.
...
VCP – Connection Pool counters
==============================

``ref_hit`` – `counter` - debug

	Number of times we have found an existing conn. pool

	Number of times we have found an existing connection pool while creating
	a backend.

``ref_miss`` – `counter` - debug

	Number of times we have not found an existing conn. pool
